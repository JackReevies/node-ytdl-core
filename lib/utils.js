 ...
 * @returns {!Error}
 */
exports.playError = (info, statuses, ErrorType = Error) => {
  let playability = info.player_response && info.player_response.playabilityStatus;
  if (playability && statuses.includes(playability.status)) {
    if (playability?.errorScreen?.playerErrorMessageRenderer?.subreason?.simpleText) {
      return new ErrorType(`${playability.reason} (${playability.errorScreen.playerErrorMessageRenderer.subreason.simpleText})`);
    }
    if (info.response?.contents?.twoColumnWatchNextResults?.results?.results?.contents?.[0]?.itemSectionRenderer?.contents?.[0]?.backgroundPromoRenderer?.title?.runs?.[0]?.text) {
      return new ErrorType(info.response.contents.twoColumnWatchNextResults.results.results.contents[0].itemSectionRenderer.contents[0].backgroundPromoRenderer.title.runs[0].text)
    }
    if (playability?.errorScreen?.playerErrorMessageRenderer?.subreason?.runs?.find(o => o.text.includes("copyright"))) {
      return new ErrorType(playability.errorScreen.playerErrorMessageRenderer.subreason.runs.map(o => o.text).join(''))
    }
